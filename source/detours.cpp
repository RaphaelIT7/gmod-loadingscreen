...
void Detour::Create(Detouring::Hook* hook, const char* name, void* module, Symbol symbol, void* hook_func, unsigned int category)
{
	...
	{
		Msg("loadingscreen: Detour %s was disabled!\n", name);
		return;
	}
	...
	if (!DETOUR_ISVALID((*hook)))
		Msg("loadingscreen: Failed to detour %s!\n", name);
}
...
void Detour::ReportLeak()
{
	for (auto& [id, hooks]: g_pDetours)
		if (hooks.size() > 0)
			Msg("loadingscreen: ID %d failed to shutdown it's detours!\n", id);
}
...
static ConCommand toggledetour("loadingscreen_toggledetour", ToggleDetour, "Debug command. Enables/Disables the given detour.", 0);